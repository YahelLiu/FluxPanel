	...
	for {
		...
		if err != nil {
			...
			if m.failures == maxConsecutiveFailures {
				log.Printf("[monitor] WARNING: %d consecutive failures. If this persists, re-login.", maxConsecutiveFailures)
			}
			select {
		...
		if resp.ErrCode == errCodeSessionExpired {
			if m.getUpdatesBuf != "" {
				...
				log.Printf("[monitor] WARNING: WeChat session expired and cannot be auto-recovered. Please re-login.")
			}
			select {
